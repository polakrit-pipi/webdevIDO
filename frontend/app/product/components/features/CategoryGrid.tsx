          ...
          return (
            <div key={cat.id} onClick={() => onSelect(isSelected ? null : cat.categoryName)} 
              ...
              <div className={`w-full aspect-3/4 bg-gray-100 mb-3 overflow-hidden relative rounded-sm border-2 transition-all duration-300 ${isSelected ? 'border-black shadow-md' : 'border-gray-200 group-hover:border-gray-400 group-hover:shadow-sm'}`}>
                  {catImgObj?.url ? <Image src={getImageUrl(catImgObj.url)} alt={cat.categoryName} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" /> : <div className="flex items-center justify-center h-full text-gray-400 text-xs bg-gray-200">No Image</div>}
              </div>
